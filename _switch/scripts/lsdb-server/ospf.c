...
void alarmHandler(int signo){
    current = clock();
    printf("=== OSPF HELLO PKT SENT! [%f s] ===\n", (double)(current - begin)/CLOCKS_PER_SEC);
    write(sock_fd, (BYTE *)buf, ETHSIZE+IPHSIZE+56);
    alarm(INTERVAL);
}

int main(int argc, char ** argv){
    if(argc != 3){
        printf("USAGE: sudo ./ospf eno1 10.11.12.13\n");
        return 0;
    }
    printf("== THIS PROG WILL SEND OSPF HELLO PACKETS EVERY %d SEC ===\n", INTERVAL);
	...
	BYTE l2[14] = { 0x01, 0x00, 0x5e, 0x00, 0x00, 0x05, 0xe0, 0xd5, 0x5e, 0x70, 0x39, 0x01, 0x08, 0x00};
